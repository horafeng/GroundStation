...
- 状态：雷达解析、航迹/选择/任务、Demo编码、UDP发送调度核心已实现；UI和雷达接收待后续阶段
...
丢失持续时间使用单调时钟计算；对外时间戳使用明确时区的 Unix 时间或经确认的协议时间。
...
- `observation_received_monotonic`：本机接收判定和丢失持续时间。
- `coordinate_timestamp`：最后有效坐标所对应的时间；优先使用已确认雷达时间，否则使用本机接收 UTC 时间并标注来源。
- `lost_since_monotonic`：首次判定丢失的单调时间。
