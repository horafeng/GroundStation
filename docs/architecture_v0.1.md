...
1. 顶部紧凑状态条：雷达监听与数据状态、UDP发送且无ACK、当前模式、当前航迹和入口按钮。
2. 主区域完全交给地图/视频工作区，不设置固定左右信息栏或底部日志栏。
3. 五种任务模式以纵向“图标+文字”按钮悬浮在工作区左上角，不占布局列。
4. 当前目标以右上角浮动卡片显示；卡片使用同一实例切换紧凑/展开状态，不做尺寸动画。
5. 航迹、通信、日志、报文和协议错误进入非模态“运行检查”窗口。
...
- `MapSceneModel` 从航迹生命周期快照生成地图标记，继续以绝对编号关联，并为每条航迹
  保存有配置上限的历史点。
- `TileMapCanvas` 负责投影、异步在线瓦片、本地Demo底图和鼠标交互；它不拥有选择、任务
  或UDP规则。地图刷新通知默认合并到不高于10Hz。
- `MapWidget` 是UI使用的窄接口；地图点击回到既有人工选择服务，切换仍需二次确认。
- `MainWorkspace` 只组合地图主视角和视频占位副视角；视频失败或未连接不影响雷达与发送。
- `MissionControlOverlay`和`TargetSummaryCard`只负责显示与用户输入；模式、选择确认和发送事件
  仍由既有服务处理。浮层绑定工作区而非地图控件，因此主副视角交换不会改变位置或状态。
- `RuntimeInspectionDialog`只承载原航迹表、通信详情和日志控件；关闭窗口不会停止后台业务。
- 在线瓦片不可用时继续绘制本地示意底图、Home、目标、轨迹和状态，不做离线区域下载。
