...
交互式地图主视角、视频占位切换和独立模拟工具。仍不包含MAVLink、真实飞控、
无人机飞行动作或真实视频解码。
...
主界面只保留紧凑运行状态、航迹数量、“运行检查”和“系统设置”入口。系统设置窗口一级页签为
“系统设置”和“人机校验”，系统设置内含雷达、无人机、协议与航迹、地图、视频与存储。
人机校验明确显示当前未接入飞控双向协议，不会发送命令或伪造结果。雷达监听地址、来源过滤和字节序
需要停止并重新启动雷达监听才生效；无人机端点、ID和频率需要停止并重新启动发送才
生效；航迹超时阈值点击“应用”后立即生效。配置字段含义及文件格式保持兼容。

主窗口采用地图作战型布局：地图/视频工作区占据状态条以下的全部空间，五种任务按钮以
“图标+文字”纵向悬浮在左上角，当前目标以紧凑浮动卡片显示在右上角。卡片点击后直接切换
明细可见性，不重建控件、不使用尺寸动画；地图/视频交换、航迹刷新和目标丢失不会自动收起。
地图左下角保留视频画中画，单击可交换主副视角。

航迹表、通信详情、运行日志、报文和协议错误已移入非模态“运行检查”窗口，窗口关闭只隐藏
展示，不停止后台接收或发送。航迹表不提供“定位当前目标”按钮，选中关系仍按绝对编号保持，
周期刷新继续保留用户手动滚动位置。运行检查、设置、表格、输入框和滚动条共用深色工程主题。

## 地图与视频 Demo

默认 `map_mode=local_demo`，无需网络即可显示本地网格、道路/水域示意背景、Home/雷达、
全部目标、显示编号、实时/丢失差异、选中高亮和有上限的历史轨迹。鼠标可拖动、滚轮缩放，
“回到 Home”恢复中心。地图目标点击复用现有人工选择和二次确认服务，不改变发送坐标。

切换为 `online` 后按需异步读取当前视口的 OpenStreetMap 标准瓦片，并在地图内显示
“© OpenStreetMap contributors”。瓦片失败不会阻塞 UI，也不会影响雷达或 UDP；本地 Demo
背景继续可用。在线服务政策和缓存边界见 `docs\third_party_references.md`。

地图左下保留视频画中画，单击小窗可切到视频主视角，再单击地图小窗恢复。任务按钮和目标卡片
属于工作区浮层，主副视角交换时位置和状态保持不变。视频源当前仅有“未连接”、
“测试图案”和“RTSP地址已配置但未启用解码器”三种诚实状态；不包含录像、云台或AI。
